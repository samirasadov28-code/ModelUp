...
export function ProCapTableWaterfall({ model }: ProCapTableWaterfallProps) {
  ...
  const shareholderLabel = (raw: string) => {
    if (raw === "Founder 1") return t("wf.holder_founder1");
    if (raw === "Founder 2") return t("wf.holder_founder2");
    if (raw === "Pre-seed angels") return t("wf.holder_angels");
    if (raw === "ESOP") return t("wf.holder_esop");
    if (raw === "This round investors") return t("wf.this_round_investors");
    if (raw.endsWith(" investors")) {
      const name = raw.slice(0, -" investors".length);
      return t("wf.round_investors", { name });
    }
    return raw;
  };
  ...
  const waterfall = useMemo(() => {
    ...
    stages.push({
      label: t("wf.stage_this_round", { stage: model.answers.fundingStage.replace("-", " ") }),
  ...
  }, [holders, futureRounds, model, preSeedTotal, t]);
    ...
    <div className="space-y-5">
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">
                {t("wf.preseed_total", { total: preSeedTotal.toFixed(1) })}{" "}
                {Math.abs(preSeedTotal - 100) > 0.5 && (
                  <span className="text-amber-600 font-medium">{t("wf.preseed_total_warn")}</span>
                )}
              </p>
            </div>
            <button
            ...
            {futureRounds.map((r) => (
              <div key={r.id} className="rounded-lg border border-gray-200 bg-gray-50/60 p-3 space-y-2">
                ...
                <p className="text-[10px] text-gray-400">
                  {t("wf.postmoney_note", {
                    post: fmt(r.preMoneyValuation + r.raise),
                    pct: formatPercent(
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">
          {t("wf.waterfall_title")}
        </p>
        <div className="rounded-xl border border-gray-200 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="text-left px-4 py-2 text-gray-500 font-semibold">{t("wf.col_stage")}</th>
                <th className="text-right px-4 py-2 text-gray-500 font-semibold">{t("wf.col_premoney")}</th>
                <th className="text-right px-4 py-2 text-gray-500 font-semibold">{t("wf.col_raise")}</th>
                <th className="text-right px-4 py-2 text-gray-500 font-semibold">{t("wf.col_postmoney")}</th>
                <th className="text-right px-4 py-2 text-gray-500 font-semibold">{t("wf.col_new_equity")}</th>
